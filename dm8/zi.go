...
func (G2DB g2db) fromInt64(val int64, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case BIGINT:
		...
		return Dm_build_1331.Dm_build_1521(val), nil
	case REAL:
...
func (G2DB g2db) fromFloat64(val float64, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case REAL:
		...
		return Dm_build_1331.Dm_build_1524(float32(val)), nil
	case DOUBLE:
		return Dm_build_1331.Dm_build_1527(val), nil
	case DECIMAL:
...
func (G2DB g2db) toChar(bt byte) ([]byte, error) {
	...
	bts[0], err = G2DB.getCharByNumVal((bt >> 4) & 0x0F)
	if err != nil {
		return nil, err
	}

	bts[1], err = G2DB.getCharByNumVal(bt & 0x0F)
	if err != nil {
		return nil, err
	}
...
func (G2DB g2db) fromDecimal(val DmDecimal, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case BIGINT:
		...
		ret = Dm_build_1331.Dm_build_1521(val.ToBigInt().Int64())
	case REAL:
...
func (G2DB g2db) toInt32(val int32) []byte {
	b := make([]byte, 4)
	Dm_build_1331.Dm_build_1347(b, 0, val)
	return b
}

func (G2DB g2db) toInt64(val int64) []byte {
	b := make([]byte, 8)
	Dm_build_1331.Dm_build_1352(b, 0, val)
	return b
}

func (G2DB g2db) toFloat32(val float32) []byte {
	b := make([]byte, 4)
	Dm_build_1331.Dm_build_1357(b, 0, val)
	return b
}

func (G2DB g2db) toFloat64(val float64) []byte {
	b := make([]byte, 8)
	Dm_build_1331.Dm_build_1362(b, 0, val)
	return b
}
